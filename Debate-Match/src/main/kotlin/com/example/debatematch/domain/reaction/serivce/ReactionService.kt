...
class ReactionService(
    ...
    fun execute(request: ReactionRequest){
        ...
        val reaction = if (reactionRepository.existsByDebateIdAndUserId(request.debateId, user.id!!)) reactionRepository.findByDebateIdAndUserId(request.debateId, user.id!!) else Reaction(debateId = request.debateId, userId = user.id!!, request.reaction)
        reaction!!.update(request.reaction)
    }
}