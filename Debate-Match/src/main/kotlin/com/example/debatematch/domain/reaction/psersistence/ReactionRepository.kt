...
interface ReactionRepository: JpaRepository<Reaction, UUID> {
    fun countByDebateIdAndReaction(debateId: UUID, reaction: DebateSide): Int
    fun findByDebateIdAndUserId(debateId: UUID, userId: UUID): Reaction?
    fun existsByDebateIdAndUserId(debateId: UUID, userId: UUID): Boolean
}